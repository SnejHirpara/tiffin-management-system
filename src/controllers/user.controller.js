...
import {
  updateFileOnAppwrite,
  uploadFileOnAppwrite,
...
const updateAvatar = asyncHandler(async (req, res) => {
  const existingAvatarId = req?.user?.avatar
    ?.match(/(files)\/.*\//g)
    .join("")
    .split("/")[1];
  ...
  const updatedAvatarURL = await updateFileOnAppwrite(
    existingAvatarId,
    newAvatar
  );
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          tiffins,
          "Tiffins for logged in user fetched successfully"
        )
      );
  } catch (error) {
    throw new ApiError(
      500,
      "Server Error while fetching tiffins for logged in user"
    );
  }
});
  ...
  try {
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          usersForLoggedInAdmin,
          "All users for current logged in admin fetched successfully."
        )
      );
  } catch (error) {
    throw new ApiError(
      500,
      "Error while fetching users for currently logged in Admin."
    );
  }
});

const getNetTotalTiffinsInfoForAMonthAndYear = asyncHandler(
  async (req, res) => {
    if (!req?.user?._id) {
      throw new ApiError(401, "Unauthorized request");
    }
    ...
    if (!userId || !month || !year) {
      throw new ApiError(
        400,
        "Invalid payload - userId and month are required"
      );
    }
    ...
    try {
      ...
      if (UserRole.isAdmin(userWithUserRole.role)) {
        throw new ApiError(
          400,
          "Provided userId is of Admin. User as Admin doesn't have tiffins. So, userId must be of User Role."
        );
      }

      month = month.charAt(0).toUpperCase() + month.slice(1).toLowerCase();
      const monthStartDate = moment
        .utc(`${year}-${month}`, "YYYY-MMMM")
        .startOf("month")
        .startOf("day")
        .toDate();
      ...
      res
        ...
        .json(
          new ApiResponse(
            200,
            netTotalInfo[0] || {},
            "Total amount for taken tiffins for a month is calculated successfully"
          )
        );
    } catch (error) {
      throw new ApiError(
        500,
        "Error while fetching net total tiffins info for a user for a particular month"
      );
    }
  }
);
